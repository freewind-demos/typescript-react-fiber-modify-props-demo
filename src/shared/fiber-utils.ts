 ...
 *
 * 原理说明：
 * 这是最核心的"黑科技"部分。我们直接修改 Fiber 节点的 memoizedProps 属性。
 *
 * React 的渲染流程：
 * 1. 首先处理 pendingProps，将其传递给组件函数
 * 2. 组件执行后，结果存入 memoizedProps
 * 3. React 通过对比 memoizedProps 和 pendingProps 来决定是否重新渲染
 *
 * 我们直接修改 memoizedProps，理论上可以改变组件接收到的 props。
 *
 * 重要限制：
 * 1. 这种方式不会触发 React 的重新渲染！修改后组件不会自动更新
 * 2. React 下次渲染时可能会用 pendingProps 覆盖我们的修改
 * 3. 这是内部 API，React 版本升级后可能失效
 * 4. 无法访问组件的 state，只能修改 props
 *
 * 实际应用场景：
 * - 调试：查看组件接收到的 props
 * - 快速原型：临时修改某些 props 看看效果
 * - 高级场景：结合 React DevTools 的内部 API 实现更复杂的操作
 *
 * @param fiber - 要修改的 Fiber 节点
 * @param newProps - 新的 props 对象
 */
export function updateFiberProps(fiber: Fiber, newProps: Record<string, unknown>): void {
  if (!fiber) return;
  
  // 修改 memoizedProps - 这是组件上次渲染使用的 props
  fiber.memoizedProps = { ...fiber.memoizedProps, ...newProps };
  
  // 同时修改 pendingProps - 这是即将传递给组件的 props
  // 只有当 React 正在处理这个 fiber 时，pendingProps 才有意义
  if (fiber.pendingProps) {
    fiber.pendingProps = { ...fiber.pendingProps, ...newProps };
  }
  
  // 注意：这里没有触发任何更新！
  // 如果需要强制更新，需要调用 React 内部的调度器，这更加复杂
}
